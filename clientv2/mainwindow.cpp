...
QJsonObject MainWindow::getInput()
{
    ...
    if(name == "" || pwd == "") return QJsonObject();
    pwd = QString::fromUtf8(pwd.toUtf8().toBase64());
...
void MainWindow::on_pushButtonLogin_clicked()
{
    ...
    // 通过socket写出msg
    if(userdata["username"] == m_username){
        return;
    }
    m_websock->sendLoginSignupMsg(userdata, 0);
...
void MainWindow::on_pushButtonSignUp_clicked()
{
    ...
    // 通过socket写出msg
    m_websock->sendLoginSignupMsg(userdata, 1);
    return;
}
...
void MainWindow::on_pushButtonUpload_clicked()
{
    ...
    for (int i = 0; i < strList.size(); ++i)
    {
        ...
        m_websock->uploadImg(m_username, fileInfo.fileName(), base64Data);
    }
    // 清空list和model
